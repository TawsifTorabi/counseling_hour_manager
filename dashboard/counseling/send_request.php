						...
						<div class="col-sm-12">
							...
							<div class="row">
									<div class="col-sm-12">																								
										...
										<div class="col-sm-7">
											<h3><i class="fa-solid fa-laptop"></i> Confirm Request Counseling Hour</h3>
											Date:
										...
										<?php if($Availability && !$currentUserswithFaculty && !$currentUserswithOtherFaculty){ 
												...
												if($submissionCount <= 0){
													mysqli_query($con,"
														INSERT INTO `academic_counseling_requests`(`student_id`, `faculty_id`, `hour_id`, `trimester_id`, `request_date`, `course_code`, `section`, `status`) 
														VALUES ('$current_user_student_id', '$courseFacultyID', '$GetHourId', '$CurrentTrimesterID', '$GetRequestDate', '$GetCourseCode', '$GetSection', 'pending')
													");
												}
										?>										
												<style>
												...
												<div class="col-sm-4">
													...
													<span id="selectedALongwith" style="display: none;">
														<span style="display: inline-block; vertical-align: middle;">
															<i class="fas fa-list-ol"></i> <span id="alongwithstudentID"></span><br>
															<i class="fas fa-user-graduate"></i> <span id="alongwithstudentName"></span>
														</span>
														&nbsp;
														<span style="display: inline-block; vertical-align: middle;">
													...
													<input type="text" class="form-control" onkeyup="document.getElementById('topicData').value = this.value" placeholder="Write Topic..."/>
													<hr>
												...
												<div class="col-sm-8">
													<form name="tokenCreate" method="post" id="updateForm" action="send_request.php?update=true&hour_id=<?=$GetHourId;?>&request_date=<?=$GetRequestDate;?>&faculty_id=<?=$courseFacultyID;?>&course_code=<?=$GetCourseCode;?>&section=<?=$GetSection;?>">
														<h4>What is Your Problem?</h4>
														<textarea placeholder="Write Down Your Problems, Attach Images If Required..." id="problem"></textarea>
														<input type="hidden" name="hourID" value="<?= $GetHourId; ?>"/>
														<input type="hidden" name="courseFacultyID" value="<?= $courseFacultyID; ?>"/>
														<input type="hidden" name="courseSection" value="<?= $courseSection; ?>"/>
														<input type="hidden" name="courseCode" value="<?= $courseCode; ?>"/>
														<input type="hidden" name="alongwithStudentID" value="0"/>
														<input type="hidden" name="problem" id="textData" value="0"/>
														<input type="hidden" name="topic" id="topicData" value=""/>
														</br>
														<button type="button" onclick="PostNow()" class="btn btn-success" style="position: fixed;right: 24px;top: 1%;z-index: 9998;font-size: 19px;">Confirm Request</button>
													</form>
												</div>	
										<?php }else{ ?>
											<h3>Slot is already booked or pending!</h3>
										<?php } ?>							
										</div>
