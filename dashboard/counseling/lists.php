							...
							<div class="row">
								...
								<div class="col-sm-12">
									<div class="well">
										...
										$ListID = mysqli_real_escape_string($con, $_GET['id']);
										...
										if (!$result) {
											...
											if ($row1 = mysqli_fetch_array($result)) {
												?>
													<h1 id="listtitle"><i class="fa-solid fa-laptop"></i> <?php echo $row1['list_name']; ?></h1>
													<p><?php echo $row1['list_description']; ?></p>
													<h4 id="listtype">
														<?php  if($row1['list_type'] == 'playlist'){ ?>
															<i class="fas fa-play-circle"></i> Playlist
														<?php }if($row1['list_type'] == 'booklist'){ ?>
															<i class="fas fa-book"></i> Booklist
														<?php } ?>
													</h4>
											<?php
											}
										...
										<table id="dataTable" class="table table-hover table-dark"
											...
											<tbody id="hahaha">
												...
												$sql = " SELECT 
														...
														INNER JOIN `library_list` 
														ON `library_contents`.`list_id` = `library_list`.`list_id` 
														WHERE `library_contents`.`list_id` = $ListID
														ORDER BY `content_id`";
												$result = mysqli_query($con, $sql);
												if (!$result) {
													echo mysqli_error($con);
