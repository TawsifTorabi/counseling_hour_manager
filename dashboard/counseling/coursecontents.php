...
if (mysqli_num_rows(mysqli_query($con, "select * from sessions where session_id='$getsessionID' AND validity='$validity'")) > 0) {
			...
			<div class="row content">
				<div class="col-sm-3 sidenav hidden-xs">
					...
					<ul class="nav nav-pills nav-stacked">
						...
						<li class="active"><a href="coursecontents.php"><i class="fa-solid fa-laptop"></i> Course
								Contents</a></li>
				...
				<div class="col-sm-9 main">
					...
					<div class="row">
						...
						<div class="col-sm-12">
							<p>
							<div class="row">
								<h1><i class="fa-solid fa-laptop"></i> Course Content</h1></br>
								<?php
								...
								$CurrentUserCoursesSQL = "SELECT * FROM `academic_student_courses` 
						...
						AND `academic_student_courses`.`student_id`=$CurrentUserID";
								...
								if ($CurrentUserCoursesResult = mysqli_query($con, $CurrentUserCoursesSQL)) {
									while ($CurrentUserCoursesSQLValue = mysqli_fetch_assoc($CurrentUserCoursesResult)) {
										...
										array_push($CurrentUsersCourses_Array, array($courseCode, $courseName));
									}
								} else {
									// Handle error here
									echo "Error: " . mysqli_error($con);
									...
									<div class="well">
										<p>
											...
											<span>
												<b>Courses:</b> </br>
												<ul class="list-group">
													<?php foreach ($CurrentUsersCourses_Array as $course): ?>
														<li style="cursor: pointer"
															onclick="SearchContents('<?= $course[0] ?>')"
															class="list-group-item"><?php echo $course[0] . ' - ' . $course[1]; ?></li>
													<?php endforeach; ?>
												</ul>
											</span></br>
										</p>
								...
								<div class="col-sm-8">
									<div class="well">
										<p>
										<h4>If You have something,</h4>
										<h3>Upload Ebook, Notes or Papers or Videos and Lectures!</h3>
										<div style="scale: 1.1; margin-left: 25px;">
											<a style="margin-left: 10px;" class="button-boot" href='upload.php?type=pdf'><i
													class="fa-solid fa-upload"></i> Upload PDF</a>
											<a style="margin-left: 10px;" class="button-boot"
												href='upload.php?type=video'><i class="fa-solid fa-upload"></i> Upload
												Video</a>
										</div>
										<h3>Search Course Contents</h3>
								...
								<div class="well">
									...
									<table id="dataTable" class="table table-hover table-dark"
										...
										<tbody id="hahaha">
											...
											$queryString = $CurrentUsersCourses_Array[3][0];
											$sql = "	SELECT 
													...
													<tr>
														<td style="font-weight:bold; text-align:center;color:#dc2323;">
															...
															if ($rows['filetype'] == "video") { ?>
																<i style="font-size: 48px; color: skyblue;"
																	class="fa-solid fa-file-video"></i>
															<?php } ?>
														</td>
														<td style="width: 47vh;">
															<a href="content.php?id=<?php echo $rows['content_id'] ?>"><span
																	style="font-size:14px; font-weight: bold;">
																	<?php echo $rows['content_name'] ?>
																</span></a></br>
															<p><small>
														...
														<td>
															<?php if ($rows['filetype'] == "pdf") { ?>
																<a class="button-2"
																	href="file.php?content_id=<?php echo $rows['content_id']; ?>"
																	download><i class="fa-solid fa-download"></i> Download</a>
																<a class="button-2" href="#"
																	onclick="aurnaIframe('preview.php?content_id=<?php echo $rows['content_id']; ?>')"><i
															...
															if ($rows['filetype'] == "video") { ?>
																...
																<a class="button-2" href="#"
																	onclick="aurnaIframe('preview.php?content_id=<?php echo $rows['content_id']; ?>')"><i
																		class="fa-solid fa-file-video"></i> Watch</a>
															<?php } ?>
														</td>
														<td>
															<?php
															...
															if (mysqli_num_rows($BookmarkQuery) > 0) {
																?>
																<button class="button-2" id="bookmark<?php echo $rows['content_id']; ?>"
																	onclick="toggleBookmark(<?php echo $rows['content_id']; ?>)"
																	title="Saved"><i class="fa-solid fa-bookmark"></i></button>
															<?php } else { ?>
																<button class="button-2" id="bookmark<?php echo $rows['content_id']; ?>"
																	onclick="toggleBookmark(<?php echo $rows['content_id']; ?>)"
																	title="Save to Bookmarks"><i
							...
							<div class="col-sm-3">
								<div class="well">
														...
														<a href="lists.php?id=<?php echo $rows['list_id']; ?>">
															<div class="panel-heading">
																...
																<span style="font-size: 14px; font-weight: bold;">
																	<?php echo $rows['list_name']; ?>
																</span>
																<p><small>
																		<?php echo $rows['list_description']; ?>
																	</small></p>
															</div>
														</a>
													</div>
									...
									<a style="margin-bottom: 4px;" href="playlists.php"><i class="fa-solid fa-list"></i>
										Browse Playlists</a></br>
									</p>
								</div>
							</div>
							...
							<div class="col-sm-3">
								<div class="well">
									...
									<ul id="booklists" class="nav nav-pills nav-stacked">
										<?php
										mysqli_set_charset($con, "utf8");
										$sql = "	SELECT * FROM `library_list` 
													WHERE `list_type`='booklist' 
													AND (`library_list`.`list_name` LIKE '%" . $queryString . "%' OR `library_list`.`list_description` LIKE '%" . $queryString . "%')
													ORDER BY `list_id`";
										$result = mysqli_query($con, $sql);
										if (!$result) {
									...
									<a style="margin-bottom: 4px;" href="booklists.php"><i class="fa-solid fa-list"></i>
										Browse Booklists</a></br>
									</p>
								</div>
							</div>
							</p>
	...
	<script>
		...
		function SearchTable() {
				...
				if (this.readyState === 4 && this.status === 200) {
					...
					*/
					...
					for (let i = 0; i < content.length; i++) {
						...
						listItem.innerHTML = "<div class=\"panel panel-primary\">"+
												"<a href=\"lists.php?id="+content[0].list_id+"\">"+
													"<div class=\"panel-heading\">"+
														"<i class=\"fa-solid fa-arrow-right-from-bracket\"></i>"+
														"<span style=\"font-size: 14px; font-weight: bold;\"> "+
															content[0].list_name+
				...
				if (this.readyState === 4 && this.status === 200) {
					...
					*/
					...
					for (let i = 0; i < content.length; i++) {
						...
						// Append the list item to the unordered list
						booklists.appendChild(listItem);
			...
			xhr.open('GET', 'ajax.php?data=coursecontents&q=' + SearchQuery, true);
			xhr2.open('GET', 'ajax.php?data=coursecontentsplaylists&q=' + SearchQuery, true);
			xhr3.open('GET', 'ajax.php?data=coursecontentsbooklists&q=' + SearchQuery, true);
			xhr.send();
			xhr2.send();
			xhr3.send();
		}
